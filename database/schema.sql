...
-- ---------------------------------------------------------------------
-- repair_process : ติดตามการดำเนินการซ่อมหลังอนุมัติ
-- (เตรียมไว้สำหรับอนาคต ยังไม่มีไฟล์ในระบบที่ใช้งานตารางนี้)
-- ---------------------------------------------------------------------
CREATE TABLE `repair_process` (
    `repair_id`                 BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `request_id`                INT             NOT NULL,      -- FK -> repair_requests.request_id
    ...
    `status_repair`             VARCHAR(50)     DEFAULT NULL,
