...
if (
    $is_student &&
    !empty($user['student_classroom_id'])
) {
...
//
// นักเรียน:
//      เฉพาะรายการของตัวเอง
//
// อาจารย์:
//      แสดงทั้งหมด
//
// สำคัญ:
//      รายการ approved_by IS NULL
//      จะสามารถกดอนุมัติได้
// =====================================================
...
if ($is_staff) {
    ...
    $sql = "
        SELECT
            ...
            ust.staff_id AS requester_staff_id,
            ust.title_name AS requester_staff_title,
        ...
        LEFT JOIN user_accounts ua
            ON ua.user_id = r.requester_id

        LEFT JOIN user_students us
            ON us.user_id = r.requester_id

        LEFT JOIN user_staffs ust
            ON ust.user_id = r.requester_id

        ORDER BY

            CASE
                WHEN r.approved_by IS NULL
                THEN 0
                ELSE 1
            END,
...
if (!$stmt) {
    die(
        "เกิดข้อผิดพลาด SQL รายการแจ้งซ่อม: "
        . $conn->error
    );
}
...
<div class="page-heading">
    ...
    <div>
        ...
        <p>
            กรุณากรอกข้อมูลอุปกรณ์หรือสถานที่ที่ต้องการแจ้งซ่อม
        </p>
...
<div class="card-title">
    ...
    <div>
        ...
        <p>
            กรอกข้อมูลให้ครบถ้วนเพื่อส่งคำขอแจ้งซ่อม
        </p>
...
<?php if ($is_student): ?>
    ...
    <input
        type="text"
        value="<?= e(
            $student_classroom['classroom_number_code']
            ?? 'ไม่พบห้องเรียน'
        ) ?>"
        readonly
    >
    ...
    <input
        ...
        name="classroom_id"
        value="<?= e(
            $student_classroom['classroom_id']
            ?? ''
        ) ?>"
    >
    ...
    >
        ...
        <?php foreach (
            ...
            >
                ...
                <?= e(
                    $classroom['classroom_number_code']
                ) ?>

                <?php if (
                    !empty($classroom['building'])
                ): ?>
...
<div class="upload-box">
    ...
    <div class="upload-text">
        ...
        <p>
            สามารถแนบรูปภาพอุปกรณ์หรือปัญหาที่พบ
        </p>

        <span>
            รองรับ JPG, JPEG, PNG ขนาดไม่เกิน 5 MB
        </span>
...
<div class="notice-box">
    ...
    <div>
        ...
        <strong>
            ขั้นตอนหลังจากส่งแจ้งซ่อม
        </strong>

        <p>
            รายการแจ้งซ่อมจะอยู่ในสถานะ
            "รอครูอนุมัติ"
            จนกว่าจะมีอาจารย์อนุมัติ
        </p>
...
>
    ...
    <div style="margin-top:5px;">
        ...
        รายการที่มีสถานะ
        <strong>รอครูอนุมัติ</strong>
        สามารถกดปุ่มอนุมัติได้จากตารางด้านล่าง
...
// approved_by IS NULL
//
// ไม่ต้องเช็ค staff_id
// ไม่ต้องเช็ค advisor_staff_id
// ไม่ต้องเช็ค classroom
// =====================================================
